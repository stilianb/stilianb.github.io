  ...
  return (
    ...
    >
      <a
        ...
        } bg-surface0 hover:bg-surface1 p-1 flex gap-2 rounded-md absolute left-0 bottom-0 m-2 transition-all`}
      >
      ...
      <div className={`${isActive === "open" ? "" : ""} p-4 flex gap-4`}>
        ...
        <div className="flex flex-col gap-1">
          <div className="text-sm ">
            {props.contrib ? "Contributors:" : null}
          </div>
          <div className="truncate">
            {props.contrib
              ? props.contrib.map((contributor) => (
                  <div className="text-xs">{contributor}</div>
                ))
              : null}
          </div>
        </div>
